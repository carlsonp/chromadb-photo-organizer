...
def get_imgs(directory: str, existingids: list, patterns: list = ['*.[jJ][pP][gG$]', '*.[pP][nN][gG$]', '*.[wW][eE][bB][pP$]'], recursive: bool = True):
    path = Path(directory)
    # jpg, png, webp, etc.
...
def convertVideoFormat(app, searchpattern: str, sourcetype: str, endtype: str, parameters: str, deletesource: bool = False):
    # glob searching uses unix shell matching NOT regex
    for vid in get_videos("/static/images", [], [searchpattern]):
        if (not Path(f"{vid.removesuffix(sourcetype)}{endtype}").is_file()):
            ffmpeg_results = subprocess.run(f"ffmpeg -i {vid} -hide_banner -loglevel error {parameters} {vid.removesuffix(sourcetype)}{endtype}", capture_output=True, shell=True)
